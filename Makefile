...
.PHONY: help install assets assets-watch run dev lint format check test test-all \
...
assets: ## Build the Tailwind/daisyUI stylesheet and vendor htmx
	npm run build

assets-watch: ## Rebuild the stylesheet on every template change
	npm run watch:css
